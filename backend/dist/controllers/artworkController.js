...
exports.getReelsFeed = exports.getSavedArtworks = exports.saveArtwork = exports.likeArtwork = exports.deleteArtwork = exports.updateArtwork = exports.getArtworkById = exports.getArtworks = exports.createArtwork = void 0;
...
const createArtwork = async (req, res) => {
    try {
        ...
        const imageUploads = await Promise.all(files.map(async (file) => {
            ...
            return (0, cloudinary_1.uploadToCloudinary)(dataURI, 'artbid-hub/artworks');
        }));
        const artwork = await Artwork_1.Artwork.create({
            userId: req.user.id,
            title,
            description,
            images: imageUploads,
...
const getArtworks = async (req, res) => {
    try {
        ...
        const artworks = await Artwork_1.Artwork.find(filter)
            .populate('userId', 'username displayName avatar isVerified')
            .sort(sort)
            .skip((Number(page) - 1) * Number(limit))
            .limit(Number(limit))
            .lean();
        const total = await Artwork_1.Artwork.countDocuments(filter);
        res.json({
            artworks,
            pagination: {
                page: Number(page),
...
const getArtworkById = async (req, res) => {
    try {
        const { id } = req.params;
        const artwork = await Artwork_1.Artwork.findById(id)
...
const deleteArtwork = async (req, res) => {
    try {
        ...
        await Promise.all(artwork.images.map((img) => (0, cloudinary_1.deleteFromCloudinary)(img.publicId)));
...
const getSavedArtworks = async (req, res) => {
    try {
        ...
        const artworks = await Artwork_1.Artwork.find({ saves: req.user.id })
